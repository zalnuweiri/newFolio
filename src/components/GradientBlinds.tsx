...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Mouse spotlight effect - creates a reveal mask
    vec2 mouseUv = vec2(iMouse.x/iResolution.x, iMouse.y/iResolution.y);
    float dist = length(uv0 - mouseUv);
    float radius = max(uSpotlightRadius, 0.01);
    float spotIntensity = 1.0 - smoothstep(0.0, radius, dist);
    spotIntensity = pow(spotIntensity, uSpotlightSoftness);
    
    // Use spotlightOpacity as the "darkness" level outside the spotlight
    // When opacity is high (e.g., 1.0), outside is completely black
    // spotIntensity: 1.0 = inside spotlight, 0.0 = outside spotlight
    float revealMask = mix(1.0 - uSpotlightOpacity, 1.0, spotIntensity);
    
    // Blind stripes
    float stripe = fract(uvMod.x * max(uBlindCount, 1.0));
    if (uShineFlip > 0.5) stripe = 1.0 - stripe;
    vec3 blindEffect = vec3(stripe * 0.1);

    // Apply the reveal mask to the gradient
    vec3 col = base * revealMask - blindEffect;
